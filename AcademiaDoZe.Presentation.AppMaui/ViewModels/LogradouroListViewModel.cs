...
namespace AcademiaDoZe.Presentation.AppMaui.ViewModels
{
    public partial class LogradouroListViewModel : BaseViewModel
    {
        ...
        private async Task AddLogradouroAsync()
        {
            try
            {
                // Navega para a página de cadastro (novo logradouro)
                await Shell.Current.GoToAsync(nameof(Views.LogradouroPage));
            }
            catch (Exception ex)
            {
        ...
        private async Task EditLogradouroAsync(LogradouroDTO logradouro)
        {
            ...
            // Passa o Id via query string para a página de edição
            await Shell.Current.GoToAsync($"{nameof(Views.LogradouroPage)}?Id={logradouro.Id}");
        }
        ...
        [RelayCommand]
        private async Task SearchLogradouroAsync()
        {
            if (IsBusy)
                return;
