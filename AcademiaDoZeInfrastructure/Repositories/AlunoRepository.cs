    ...
    public class AlunoRepository : BaseRepository<Aluno>, IAlunoRepository
    {
        ...
        public override async Task<Aluno> Atualizar(Aluno entity)
        {
            try
            {
                ...
                string query = $"UPDATE {TableName} SET cpf = @Cpf, telefone = @Telefone, nome = @Nome, nascimento = @Nascimento, " +
                               "email = @Email, logradouro_id = @LogradouroId, numero = @Numero, complemento = @Complemento, senha = @Senha, foto = @Foto " +
                               "WHERE id_aluno = @Id";
                ...
                if (await command.ExecuteNonQueryAsync() == 0)
                    throw new InvalidOperationException($"Nenhum aluno encontrado com ID {entity.Id}.");

                return entity;
            }
            catch (DbException ex)
            {
                throw new InvalidOperationException($"Erro ao atualizar aluno ID {entity.Id}: {ex.Message}", ex);
            }
        }
        ...
        // Mapeia o resultado do banco para um objeto Aluno
